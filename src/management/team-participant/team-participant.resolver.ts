...
export class TeamParticipantResolver {
  ...
  @Query(() => TeamParticipant, { name: 'FIND_TP_BY_PARTICIPANT_AND_TEAM' })
  findRolesByParticipantAndTeam(@Args('participant_id') participant_id: number, @Args('team_id') team_id: number) {
    return this.teamParticipantService.findTPByParticipantAndTeam(TeamParticipantMSG.FIND_TP_BY_PARTICIPANT_AND_TEAM, participant_id, team_id);
  }
}